...
var connection = mysql.createConnection({
   ...
   /* host     : '104.238.98.227',
    user     : 'spiritco_develop',
    password : 'P@ss0987',
    database : 'spiritco_developer'*/


    host     : 'spiritconvention.com',
    user     : 'spiritconvention',
    password : 'P@ss08642',
    database : 'spiritco_developer'
});
...
app.post('/addvendor',function(req,resp){  //TYPE=1
    ...
    var year_2018 = req.body.year_2018;
    var year_2019 = req.body.year_2019;
    connection.query("INSERT into vendor_info VALUES (NULL,'"+booth_no+"','"+business_name+"','"+person_name+"','"+email+"','"+description+"','"+website+"','"+phone+"','"+year_2018+"','"+year_2019+"')", function (err, rows, fields) {
        if (!err) {
...
app.get('/vendorlist',function (req,resp) {
    connection.query("SELECT * FROM vendor_info", function (err, rows, fields) {
        if (err) {
            //  console.log(err);
...
app.post('/vendorlist1',function (req,resp) {
    let query_is;
    if(req.body.selected_year=='2018'){
        query_is = "SELECT * from vendor_info where year_2018=1";
    }
    if(req.body.selected_year=='2019'){
        query_is = "SELECT * from vendor_info where year_2019=1";
    }
    connection.query(query_is, function (err, rows, fields) {
        if (err) {
            resp.send(JSON.stringify({'res':[]}));
        } else {
            resp.send(JSON.stringify({'res':rows}));
...
app.post('/deletevendor', function (req, resp) {
    var id = req.body.id;
    connection.query(" DELETE FROM vendor_info WHERE id="+id, function (err, rows, fields) {
        if (err){
            resp.send("failed");
            throw err;
        }
...
app.post('/editvendor',function(req,resp){
    var booth_no = req.body.booth_no;
    var business_name = req.body.business_name;
    var person_name = req.body.person_name;
    var email = req.body.email;
    ...
    connection.query("UPDATE vendor_info SET business_name='"+business_name+"',person_name='"+person_name+"',email='"+email+"',description='"+description+"',website='"+website+"',phone='"+phone+"',year_2018='"+year_2018+"',year_2019='"+year_2019+"' WHERE id="+id, function (err, rows, fields) {
        if (err) {
...
app.post('/vendordetails',function(req,resp){        // this is for editadmin page
    var resitem = {};
    var id =req.body._id;

    connection.query("SELECT * FROM vendor_info WHERE id="+id, function (err, rows, fields) {
        if (err) {
            resp.send(JSON.stringify({'status':'error','id':0}));
        } else {
            resitem = rows[0];
...
app.post('/addhealer',function(req,resp){  //TYPE=1
    ...
    var year_2018 = req.body.year_2018;
    var year_2019 = req.body.year_2019;
    connection.query("INSERT into healer_info VALUES (NULL,'"+table_no+"','"+business_name+"','"+email+"','"+description+"','"+website+"','"+phone+"','"+year_2018+"','"+year_2019+"')", function (err, rows, fields) {
        if (!err) {
            resp.send(JSON.stringify({'status':'success','id':rows.insertId}));
        }
        else {
...
app.get('/healerlist',function (req,resp) {
    connection.query("SELECT * FROM healer_info", function (err, rows, fields) {
        if (err) {
            //  console.log(err);
            resp.send(JSON.stringify({'res':[]}));
        } else {
...
app.post('/healerlist1',function (req,resp) {
    let query_is;
    if(req.body.selected_year=='2018'){
        query_is = "SELECT * from healer_info where year_2018=1";
    }
    if(req.body.selected_year=='2019'){
        query_is = "SELECT * from healer_info where year_2019=1";
    }
    connection.query(query_is, function (err, rows, fields) {
        if (err) {
            resp.send(JSON.stringify({'res':[]}));
...
app.post('/deletehealer', function (req, resp) {
    var id = req.body.id;
    connection.query(" DELETE FROM healer_info WHERE id="+id, function (err, rows, fields) {
        if (err){
            resp.send("failed");
            throw err;
        }
...
app.post('/edithealer',function(req,resp){
    var booth_no = req.body.booth_no;
    var business_name = req.body.business_name;
    var email = req.body.email;
    var description = req.body.description;
    ...
    connection.query("UPDATE healer_info SET business_name='"+business_name+"',email='"+email+"',description='"+description+"',website='"+website+"',phone='"+phone+"',year_2018='"+year_2018+"',year_2019='"+year_2019+"' WHERE id="+id, function (err, rows, fields) {
        if (err) {
            resp.send(JSON.stringify({'status':'error'}));
        } else {
            resitem = rows[0];
...
app.post('/healerdetails',function(req,resp){        // this is for editadmin page
    var resitem = {};
    var id =req.body._id;

    connection.query("SELECT * FROM healer_info WHERE id="+id, function (err, rows, fields) {
        if (err) {
            resp.send(JSON.stringify({'status':'error','id':0}));
        } else {
            resitem = rows[0];
...
app.post('/addspeaker',function(req,resp){
    var speaker_name = req.body.speaker_name;
    var description = req.body.description;
    var servername = req.body.servername;
    var uploadedfilename = req.body.uploadedfilename;
    var subjects_covered = req.body.subjects_covered;
    var start_date_time = req.body.start_date_time;
    var end_date_time = req.body.end_date_time;
    var year_2018 = req.body.year_2018;
    var year_2019 = req.body.year_2019;
    connection.query("INSERT into speaker_info VALUES (NULL,'"+speaker_name+"','"+mysql_real_escape_string(description)+"','"+servername+"','"+uploadedfilename+"','"+subjects_covered+"','"+start_date_time+"','"+end_date_time+"','"+year_2018+"','"+year_2019 +"')", function (err, rows, fields) {
        console.log(rows);
        console.log(err);
        if (!err) {
            resp.send(JSON.stringify({'status':'success','id':rows.insertId}));
...
app.get('/speakerlist',function (req,resp) {
    connection.query("SELECT * from speaker_info", function (err, rows, fields) {
        if (err) {
            resp.send(JSON.stringify({'res':[]}));
        } else {
            resp.send(JSON.stringify({'res':rows}));
...
app.post('/speakerlist1',function (req,resp) {
    let query_is;
    if(req.body.selected_year=='2018'){
        query_is = "SELECT * from speaker_info where year_2018=1";
    }
    if(req.body.selected_year=='2019'){
        query_is = "SELECT * from speaker_info where year_2019=1";
    }
    connection.query(query_is, function (err, rows, fields) {
        if (err) {
            resp.send(JSON.stringify({'res':[]}));
...
app.post('/deletespeaker', function (req, resp) {
    connection.query("DELETE FROM speaker_info WHERE id="+req.body.id, function (err, rows, fields) {
        if (err){
            resp.send("failed");
            throw err;
        }
...
app.post('/speakerdetails',function(req,resp){
    var resitem = {};
    var id =req.body.id;
    connection.query("SELECT * FROM speaker_info WHERE id="+id, function (err, rows, fields) {
        if (err) {
            resp.send(JSON.stringify({'status':'error','id':0}));
        } else {
            resitem = rows[0];
...
app.post('/editspeaker',function(req,resp){
    var id = req.body.id;
    var speaker_name = req.body.speaker_name;
    var servername = req.body.servername;
    var description = req.body.description;
    ...
    connection.query("UPDATE speaker_info SET servername='NULL', uploadedfilename='NULL' WHERE id="+id, function (err, rows, fields) {
        if (err) {
            console.log(err);
        } else {
            console.log('done');
...
/*    connection.query("UPDATE speaker_info SET speaker_name='"+speaker_name+"',description='"+mysql_real_escape_string(description)+"',servername='"+servername+"',uploadedfilename='"+uploadedfilename+"',subjects_covered='"+subjects_covered+"',year_2018='"+year_2018+"',year_2019='"+year_2019+"',start_date_time='"+start_date_time+"',end_date_time='"+end_date_time+"' WHERE id="+id, function (err, rows, fields) {*/
    connection.query("UPDATE speaker_info SET speaker_name='"+speaker_name+"',description='"+mysql_real_escape_string(description)+"',servername='"+servername+"',uploadedfilename='"+uploadedfilename+"',subjects_covered='"+subjects_covered+"',year_2018='"+year_2018+"',year_2019='"+year_2019+"' WHERE id="+id, function (err, rows, fields) {
        if (err) {
            resp.send(JSON.stringify({'status':'error'}));
        } else {
            resitem = rows[0];
...
app.post('/contactus',function(req,resp){
    ...
    var message_got = req.body.message;
    var phone = req.body.phone;

    connection.query("INSERT into contact_us VALUES (NULL,'"+name+"','"+email+"','"+subject+"','"+message_got+"','"+phone+"')", function (err, rows, fields) {
        if (!err) {
            resp.send(JSON.stringify({'status':'success','id':rows.insertId}));
            var smtpTransport = mailer.createTransport({
                service: "Gmail",
            ...
            var message = '<html><body><div style="width: 600px; margin: 0 auto; padding: 10px; border-radius: 10px; border: solid 5px #ff7d00; font-family: Arial; font-size: 14px; color: #333;">';
            message += '<img src="http://testbed.spiritconutah.com/assets/images/logo-spiritconversation.png" style="display: block; margin: 0 auto; margin-top: 20px;">';
            message += '<h2 style="display: block; text-align: center; padding: 15px 0; text-transform: uppercase; font-size: 22px; color: #ff7d00; margin: 0px;">User Information</h2>';
            message += '<div style="width: auto; padding: 10px; border-bottom: solid 1px #ddd; border-top: solid 1px #ddd; color: #ff7d00;">Name : <span style="color: #000;"> ' + name+ '</span></div>';
            message += '<div style="width: auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00; background: #ddd;">Email :  <span style="color: #000;">' +email+ '</span></div>';
            message += '<div style="width: auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00;">Phone : <span style="color: #000;"> ' +phone+ '</span></div>';
            message += '<div style="width: auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00; background: #ddd;">Subject : <span style="color: #000;"> ' +subject+ '</span></div>';
            message += '<div style="width: auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00;" >Message : <span style="color: #000;"> ' +message_got+ '</span></div>';
            message += '</div></body></html>';
            var mail = {
                to : 'support@brownpapertickets.com,ipsita.influxiq@gmail.com,debasiskar007@gmail.com',
                subject : 'New Form Submission from website http://spiritcon.com/ By '+name ,
                html: message,
                from: '"Spiritcon Admin" <support@spiritcon.com>',
                replyTo: 'support@spiritcon.com',
                sender: 'support@spiritcon.com',
            }
...
app.post('/newsletter',function(req,resp){
    ...
    console.log(connection.query("INSERT into newsletter VALUES (NULL,'"+email+"','"+added_date+"')"));
    connection.query("INSERT into newsletter VALUES (NULL,'"+email+"','"+added_date+"')", function (err, rows, fields) {
        if (!err) {
            resp.send(JSON.stringify({'status':'success','id':rows.insertId}));
        }
        else {
    ...
    if(typeof(id)=='undefined') {
        connection.query("INSERT into user_info VALUES (NULL,'"+first_name+"','"+last_name+"','"+personal_email+"','"+password+"','"+secondary_email+"','"+cellphone+"','"+address+"','"+city+"','"+state+"','"+postal_code+"','"+best_time+"','"+healthcare_industry_year+"','"+carrer_change+"','"+doctor_count+"','"+get_started+"','"+additional_info+"','"+background_info+"','"+greatest_attribute+"','"+add_time+"','0','NULL','1')", function (err, rows, fields) {
            ...
            if (!err) {
                ...
                message += '<h2 style="display: block; text-align: center; padding: 15px 0; text-transform: uppercase; font-size: 22px; color: #ff7d00; margin: 0px;">User Information</h2>';
                message += '<div style="width: auto; padding: 10px; border-bottom: solid 1px #ddd; border-top: solid 1px #ddd; color: #ff7d00;">Name : <span style="color: #000;"> ' + first_name+ ' ' + last_name+ '</span></div>';
                message += '<div style="width: auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00; background: #ddd;">Personal Email :  <span style="color: #000;">' +personal_email+ '</span></div>';
                message += '<div style="width: auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00;">Secondary Email : <span style="color: #000;"> ' +secondary_email+ '</span></div>';
                message += '<div style="width: auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00; background: #ddd;">Cell Phone : <span style="color: #000;"> ' +cellphone+ '</span></div>';
                message += '<div style="width: auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00;" >Address : <span style="color: #000;"> ' +address+ '</span></div>';
                message += '<div style="width: auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00; background: #ddd;">City : <span style="color: #000;"> ' +city+ '</span></div>';
                message += '<div style="width: auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00;">State : <span style="color: #000;"> ' +state+ '</span></div>';
                message += '<div style="width:auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00; background: #ddd;">Postal Code : <span style="color: #000;"> ' +postal_code+ '</span></div>';
                message += '<div style="width: auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00;">Best Time To Reach Me : <span style="color: #000;"> ' +best_time+ '</span></div>';
                message += '<div style="width:auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00; background: #ddd;">Heathcare Industry Experience   <span style="color: #000;">: ' +healthcare_industry_year+ '</span></div>';
                message += '<div style="width:auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00;">Reason For Career Change/sideline Income  :  <span style="color: #000;">' +carrer_change+'</span></div>';
                message += '<div style="width: auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00; background: #ddd;">Doctor/Clinics Relation :  <span style="color: #000;">'+doctor_count+ '</span></div>';
                message += '<div style="width:auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00;">Ready To Started : <span style="color: #000;"> ' +get_started+ '</span></div>';
                message += '<div style="width:auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00; background: #ddd;">Additional Info : <span style="color: #000;"> ' +additional_info+ '</span></div>';
                message += '<div style="width:auto; padding: 10px; border-bottom: solid 1px #ddd; color: #ff7d00;">Background In Health Industry : <span style="color: #000;"> ' +background_info+ '</span></div>';
                message += '<div style="width: auto; padding: 10px;  color: #ff7d00; background: #ddd;" >Greatest Attributes : <span style="color: #000;"> ' +greatest_attribute+ '</span></div>';
                message += '<div style="width: auto; padding: 10px;  color: #ff7d00;" ><h2 style="text-align: center; font-size: 22px; color: #ff7d00; display: block; width: 100%; padding: 0px; margin: 0px;">Login Info</h2></div>';
                message += '<div style="width: auto; padding: 10px;  color: #ff7d00; background: #ddd;" >Username : <span style="color: #000;"> '+personal_email+'</span></div>';
                message += '<div style="width: auto; padding: 10px;  color: #ff7d00;" >Password : <span style="color: #000;">'+password+'</span></div>';
                message += '</div></body></html>';
                var mail = {
...
var db;
var url = 'mongodb://localhost:27017/nexmedsolutions';
...
app.post('/contactusfornexmedical',function(req,resp){
    console.log('contactusfornexmedical');
    ...
    collection.insert([{
        ...
        if (err) {
            ...
            var mailOptions = {
                 from: "Admin <nexmedsolutions@gmail.com>",
                 to: req.body.email,
                 subject: 'Comment Added.',
                 text: 'Username: '+req.body.email +'<br/><p>Your message is submitted!</p>'
             };
...
app.post('/contactusfornexmedical1',function (request,response) {
    ...
    collection.insert([{
        ...
        if (error){
            ...
            response.send(JSON.stringify({'status':'success','id':result.ops[0]._id}));
        }
